...
export const EmailValidation = ({ onAction }: EmailValidationProps) => {
  const [email, setEmail] = useState("");
  const { sharedChartDataError } = useSharedChartData();
  return (
    <>
      {sharedChartDataError && (
        <ErrorAlert
          title="Email not valid"
          description={sharedChartDataError}
        />
      )}
      <Flex
        minH={"100vh"}
        ...
        >
          ...
          <Stack align={"center"} spacing={2}>
            ...
            <Text fontSize={"lg"} color={"gray.500"}>
              Your email is required to access this this
            </Text>
          </Stack>
          <Stack
            spacing={4}
